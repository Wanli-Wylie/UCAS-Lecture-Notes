...
分布式和并行计算本质上都涉及对系统中不同节点（具备状态，且具备算力、内存、存储资源）的调度与协调问题。

- **并行计算**。并行计算的最根本问题是：如何能把一个串行算法中涉及的数据、控制流等分配到不同的节点上，使得不同节点可以并行执行这些控制流，在需要的时候与其他节点进行通信。
...
- **分布式系统**。分布式系统其实和FSM非常相似，无论是达成共识，还是选举等，最终都落脚到不同节点的状态转移逻辑上。
